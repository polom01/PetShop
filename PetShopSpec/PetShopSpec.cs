...
namespace Training.Spec
{
    ...
    public class when_searching_for_pets : concern_with_pets_for_sorting_and_filtering
    {
        ...
        {
            Criteria<Pet> criteria = Where<Pet>.has_An(p => p.species).equalTo(Species.Cat);
        ...
        {
            var criteria = Where<Pet>.has_An(p => p.species).Not().equalTo(Species.Mouse);
            var foundPets = subject.AllPets().ThatSatisfy(criteria);
        ...
        {
            var criteria = Where<Pet>.has_An(p => p.yearOfBirth).greaterThan(2010);
            var foundPets = subject.AllPets().ThatSatisfy(criteria);
